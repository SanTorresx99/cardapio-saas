  ...
  const handleConfirmarPagamento = async () => {
    ...
    try {
      const res = await fetch(`/api/pedidos/${pedido.id}/confirmar`, {
        ...
        body: JSON.stringify({ confirmacaoToken: pedido.confirmacaoToken }),
      });
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white shadow-sm px-4 py-4">
        <h1 className="text-lg font-bold text-gray-900">Pedido #{String(pedido.id).slice(-6).toUpperCase()}</h1>
      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-4">
        {pedido.pixPayload ? (
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 text-center text-yellow-800 text-sm">
            PIX não configurado para este restaurante. Combine o pagamento diretamente.
          </div>
        )}
